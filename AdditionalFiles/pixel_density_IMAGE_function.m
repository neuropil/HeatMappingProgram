function [Gheat,Gintensity_mask_matrix,maskIndices] = pixel_density_IMAGE_function(fileName)
...
% 
% 
% picname = input('Input image file name without .tif: ', 's');
% 
% 
% if isempty(picname)
%     picname = '';
...
%%% ADD code that checks to verify if the selected image is a tiff file
...
thresholdG = mean(pixth.green_threshold) + 3*std(pixth.green_threshold);
...
set(gca,'XLim',[0 max(Gpixel_hist)])
...
xlabel('Pixel Intensity');
legend('Before Threshold','After Threshold');
set(gca,'XLim',[0 max(Rpixel_hist)])
...
set(h1, 'AlphaData', Rintensity_mask_matrix);
colorbar
hold on
