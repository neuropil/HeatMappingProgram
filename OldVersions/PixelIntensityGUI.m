function varargout = PixelIntensityGUI(varargin)
% PIXELINTENSITYGUI MATLAB code for PixelIntensityGUI.fig
%      PIXELINTENSITYGUI, by itself, creates a new PIXELINTENSITYGUI or raises the existing
%      singleton*.
%
%      H = PIXELINTENSITYGUI returns the handle to a new PIXELINTENSITYGUI or the handle to
%      the existing singleton*.
%
%      PIXELINTENSITYGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PIXELINTENSITYGUI.M with the given input arguments.
%
%      PIXELINTENSITYGUI('Property','Value',...) creates a new PIXELINTENSITYGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PixelIntensityGUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PixelIntensityGUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PixelIntensityGUI

% Last Modified by GUIDE v2.5 11-Nov-2013 19:28:26

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PixelIntensityGUI_OpeningFcn, ...
                   'gui_OutputFcn',  @PixelIntensityGUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PixelIntensityGUI is made visible.
function PixelIntensityGUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PixelIntensityGUI (see VARARGIN)

% Choose default command line output for PixelIntensityGUI
...
% UIWAIT makes PixelIntensityGUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = PixelIntensityGUI_OutputFcn(hObject, eventdata, handles) 
...
for i = 1:length(handles.fNameS)
    ...
    set(handles.FILEname,'String','DRAW and DRAG box')
    ...
    polydim = size(tempFile); % Dimensions of image file
...
if isempty(contents{get(hObject,'Value')})
    ...
    [f1,x1,~] = ksdensity(Gnumpixels2double);
    axes(handles.kdens)
    ...
    [f3,x3,~] = ksdensity(Rnumpixels2double);
    axes(handles.pHist)
    ...
    ylabel('Density');
    xlabel('Pixel Intensity');
    set(gca,'YLim',[0 max([f3,f4])]);
    hold off;
    
    
    set(handles.pDatapanel,'Visible','on')
    ...
    %     figure;
    %     h1 = image(Rheat,'CDataMapping','scaled');
    %     colormap(jet(256));
